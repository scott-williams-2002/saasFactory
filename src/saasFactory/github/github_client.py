...
class GitHubRepoClient():
    ...
    def remove_old_init(self, repo_path: str) -> bool:
        """
        ...
        try:
            if (repo_path / ".git").exists():
                subprocess.run(["rm", "-rf", f"{repo_path}/.git"], timeout=10)
                subprocess.run(["git", "init"], cwd=repo_path, timeout=10)
            return True
    ...
    def remove_upstream(self, repo_path: str) -> bool:
        """
    ...
    def add_upstream(self, repo_path: str, repo_url: str) -> bool:
        """
        ...
        """
        try:
            subprocess.run(["git", "remote", "add", "origin", repo_url], cwd=repo_path, timeout=10)
            return True
        except subprocess.TimeoutExpired as e:
            print(f"{Emojis.CLOCK.value} Command timed out: {e}")
            return False
    ...
    def add_commit_push(self, repo_path: str, repo_name: str, branchname: str = "main") -> bool:
        """
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", "saasFactory - Initial commit"], cwd=repo_path, timeout=10)
            subprocess.run(["git", "branch", "-M", branchname], cwd=repo_path, timeout=10)
            subprocess.run(["git", "push", "-u", "origin", branchname], cwd=repo_path, timeout=10)
            return True
        except subprocess.TimeoutExpired as e:
            print(f"{Emojis.CLOCK.value} Command timed out: {e}")
            return False
